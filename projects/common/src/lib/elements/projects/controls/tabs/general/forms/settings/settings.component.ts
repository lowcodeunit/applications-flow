    ...
    Object.keys(this.Form.controls).forEach(
      (key: string) => {
        ...
        if (toggle !== k && toggle.includes(k)) {
          checked ? this.Form.controls[key].enable() : this.Form.controls[key].disable();
        }
    });
  }
   ...
   */
    ...
    protected config(): void {
      this.Config = new CardFormConfigModel({
        ...
        Subtitle: 'When using a framework for a new project, it will be automatically detected. As a s result, several project settings are automatically configured t achieve the best result. You can override them below.',
        FormActions: {
